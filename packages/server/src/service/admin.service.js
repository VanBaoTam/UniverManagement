...
export class AdminService {
  ...
  async createCourse(req, res) {
    try {
      ...
      for (const item of schedule) {
        ...
        const courseMappingValue = [courseId, item.day, item.shift, teacherId];
  ...
  async getUsers(req, res) {
    ...
    if (!CredentialsValidation("id", accountId))
      return res.status(400).json({ message: "Invalid Account id" });
    const dataQuery =
      "select accounts.account_id, accounts.role_id, profiles.name, profiles.email, profiles.phone_number from accounts join profiles on accounts.profile_id = profiles.profile_id";
    ...
    return res.status(200).json({ message: "Get list successfully", userList });
  }
}
